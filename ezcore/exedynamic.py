...
def ConfigureOldStyleCgi(ExeController):
  if bfsHtml is None:
    ...
    ExeController.cgi			= bfsHtml.bfsHtml(bzHtml.MODECGI, ExeController,
  ...
  if bfsSpecFile is None:
    ...
    ExeController.catMarkup.clientSubstituteAndWriteTextFunction = bfsGenProduct.WriteDesc
    ExeController.catMarkup.clientProductFunction = bfsGenProduct.WriteProductEntry
    ExeController.catHtml		= bfsHtml.bfsHtml(bzHtml.MODEFILE, ExeController,
						ConversationId=ExeController.cgiGetData[bzHtml.CONVERSATIONKEY])
