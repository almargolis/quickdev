...
class FileInfo:  # pylint: disable=too-few-public-methods
    ...
    def new_path(self, ext):
        ...
        if ext in ["", None]:
            ...
            file_name = self.module_name + ext
...
def open_xdb(db_path, db_reset=False, debug=0):
    """Open the XSynth database."""
    if db_reset and (db_path != qdsqlite.SQLITE_IN_MEMORY_FN):
        try:
            os.unlink(db_path)
        except FileNotFoundError:
            pass
    return qdsqlite.QdSqlite(
        db_path, db_dict=xdb_dict, detailed_exceptions=True, debug=debug
    )


def post_module_table(db, file_info, file_mode):
    """Update module table with file information."""
    module_data = db.lookup(XDB_MODULES, where={"module_name": file_info.module_name})
    if file_mode == FILE_MODE_SOURCE:
        if module_data is not None:
            if (module_data["source_found"] == "Y") and (
                module_data["source_source"] != file_info.path
            ):
                abend(f"Duplicate module name {file_info.module_name}")
        uflds = {}
    ...
    db.update_insert(XDB_MODULES, uflds, where={"module_name": file_info.module_name})
...
def post_files_table(db, file_info):
    ...
    if file_info.file_ext in XSOURCE_SOURCE_EXT:
        ...
        if file_info.module_name in RESERVED_MODULE_NAMES:
            abend(f"Reserved module name {file_info.module_name}")
        is_module = True
...
class PythonParse:
    ...
    def new_module(self):
        """Initialize the start of a module."""
    ...
    def new_class(self, lex):
        """Initialize the start of a class."""
        self.class_name = lex.tokens[1]
        self.class_indent = lex.start_ixs[0]
        if (len(lex.tokens) >= 5) and (lex.tokens[2] == "(") and (lex.tokens[4] == ")"):
        ...
        fld_values = {
            "module_name": self.source_obj.module_name,
    ...
    def end_class(self):
        """Cleanup then end of a class."""
        self.class_indent = NO_INDENT
        self.class_name = ""

    def new_def(self, lex):
        """Create a new define symbol."""
...
class CodeBlock:  # pylint: disable=too-many-instance-attributes, too-few-public-methods
    """
    Container for one block of source code.
    """
    ...
    def initialize_loop(self):
        """Initialize a loop through ???."""
        self.dictionary_data = self.xsource.db.require(
            XDB_DICTS, where={"dict_name": self.dictionary_name}
        )
        self.loop_items = self.xsource.db.select(
            XDB_DICT_ELEMENTS, where={"dict_name": self.dictionary_name}
        )
        self.loop_ix = 0


class XSource:  # pylint: disable=too-many-instance-attributes
    """
    ...
    def __init__(
        ...
        # pylint thinks this needs to be simplified
        if debug > 0:
            print(
                f"XSource: {module_name}"
                f" {source_path} '{source_ext}'"
                f" {target_path} '{target_ext}'"
            )
        self.blocks = []
        self.built_ins = {}
        self.dictionary_name = None
        ...
        print(f"Processing {self.module_name} {self.source_path} -> {self.target_path}")
        self.db.delete(XDB_MODULE_USES, where={"source_module_name": module_name})
        self.db.update(
            XDB_MODULES,
        ...
        for this_line in source_lines:
            ...
            if this_line[:2] == "#$":
                ...
                if self.defines_only:
                    ...
                    self.xsynth_python(this_line)
        while len(self.blocks) > 0:
            this = self.blocks.pop()
            err_msg = f"Missing 'end' for {this.block_type} block"
        ...
        if self.debug > 0:
            print(f"XSource {self.module_name} {status}")
        self.db.update(
            XDB_MODULES, {"status": status}, where={"module_name": module_name}
        )
        if self.defines_only:
            pass
        else:
    ...
    def write_output_file(self):
        """
        Write the synthesized source file.

        The file has write priveledges revoked to
        help avoid wasting time editing the
        synthesized file.
        """
    ...
    def validate_module_name(self, module_name):
        ...
        """
        module_data = self.db.lookup(XDB_MODULES, where={"module_name": module_name})
        if module_data is None:
            self.syntax_error(f"Unknown module '{module_name}'.")
            return
    ...
    def xsynth_lookup_qdict(self, parts):
        ...
        if item_property == "as_comma_list":
            flds = []
            for this in sql_data:
                flds.append(this["element_name"])
            return ", ".join(flds)
        if item_property == "as_subst_list":
        ...
        if item_property == "as_dict_extract_list":
            ...
            for this in sql_data:
                ...
                flds.append(fld_str)
            return ", ".join(flds)
        self.syntax_error("Unknown qdict property '{item_property}'")
        return None

    def xsynth_lookup_subst(self, key):
        """Find a substituion value."""
        ...
        if len(parts) == 1:
            fld_values = {"module_name": self.module_name, "define_name": parts[0]}
        else:
            fld_values = {"module_name": parts[0], "define_name": parts[1]}
    ...
    def xsynth_subst(self, src_line, ix, ix2):
        ...
        if value is None:
            self.syntax_error(f"Unknown substituion {xpy_string}")
            return ix2 + 1, src_line
        sub_string = f"{quote}{value}{quote}"
    ...
    def syntax_error(self, msg):
        ...
        self.err_ct += 1
        print(f"{self.module_name} Line {self.source_line_ct}: {msg}")

    def xsynth_action(self, parts):
        """Process an action definition."""
        if len(parts) < 2:
        ...
        self.xsynth_python(f"class {parts[1]}({parts[2]}):")
    ...
    def xsynth_define(self, parts):
        ...
        if parts[1] in RESERVED_MODULE_NAMES:
            ...
            self.syntax_error(f"Reserved define name {parts[1]}")
            return
        where_values = {"module_name": self.module_name, "define_name": parts[1]}
        fld_values = {
            "module_name": self.module_name,
            "define_name": parts[1],
            "value": parts[2],
        ...
        try:
            ...
            self.syntax_error(f"Duplicate define {parts[1]}")

    def xsynth_dict(self, parts):
        """Create an XSynth dictionary."""
        where_values = {"dict_name": parts[1]}
        fld_values = {
            "module_name": self.module_name,
            "dict_name": parts[1],
        }
        try:
            ...
            self.syntax_error(f"Duplicate dict {parts[1]}")
        block = CodeBlock(self, "dict")
        block.block_name = parts[1]
        self.blocks.append(block)

    def xsynth_element(self, parts):
        """Process an element of an XSynth dictionary."""
        ...
        try:
            ...
            self.syntax_error(f"Duplicate dict element {parts[1]}")

    def xsynth_end(self, parts):
        """Process the end of a block."""
    ...
    def xsynth_parse(self, src_line):
        ...
        if self.defines_only:
            ...
            return
        if parts[0] == "action":
            self.xsynth_action(parts)
        elif parts[0] == "dict":
            self.xsynth_dict(parts)
        elif parts[0] == "element":
            self.xsynth_element(parts)
        elif parts[0] == "end":
            self.xsynth_end(parts)
        elif parts[0] == "for":
            self.xsynth_for(parts)
        elif parts[0] == "prog":
            self.xsynth_prog(parts)
        else:
            self.syntax_error(f"Unknown XSynth directive '{parts[0]}'")
