...
def run_and_test(tmpdir, test_src, expected_out):
    """Test XSource process."""
    db_path = tmpdir.join("db.sql")
    db = qdsqlite.QdSqlite(db_path, db_dict=xsource.xdb_dict, debug=0)
    assert db is not None
    print(db_path)
    x = xsource.XSource(
        "test", db=db, source_ext=".xpy", source_lines=test_src, target_dir=tmpdir
    )
    print(x.target_path)
    assert x.err_ct == 0
    with open(x.target_path, encoding="utf-8") as outf:
    ...
    for ix, this_line in enumerate(out_lines):
        ...
        print(f"** {this_line} ** {expected_line} **")
