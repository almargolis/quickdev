...
def MakePythonExecutionStub(parmConfiguration, ExecutableResource=None, IsHelp=False):
    def WriteCommands(parmSection):
        ...
        for wsThisCommandResource in wsCommands:
            ...
            if parmSection == "cgi":
                wsCommandSelector = bzUtil.Upper(wsThisCommandResource.selector)
            else:
                wsCommandSelector = wsThisCommandResource.switch
            wsCommandMap[wsCommandSelector] = wsThisCommandResource.commandRefname
        wsF.write(
            "    self.%s            = %s"
            % (wsPropertyName, bafExpCodeWriter.MakePythonDictLiteral(wsCommandMap))
        )
    ...
    if ExecutableResource:
        ...
        if len(ExecutableResource.commandsBySelector) >= 1:
            ...
            wsF.write(
                "import %s.%s as %s\n"
                % (
                    ExecutableResource.primaryModuleResourceObject.pythonPackageName,
                    wsModuleName,
                    wsModuleName,
                )
            )
        ...
        if IsHelp:
            ...
            for wsThisExecutableRefname in parmConfiguration.executableRefnames:
                wsThisExecutableResource = parmConfiguration.executables[
                    wsThisExecutableRefname
                ]
                if len(wsThisExecutableResource.commandsBySwitch) > 0:
                    wsF.write(
                        "  print '%s'\n" % (wsThisExecutableResource.executableFileName)
                    )
                    for wsThisCommandResource in list(
                        wsThisExecutableResource.commandsBySwitch.values()
                    ):
                        wsF.write(
                            "  print '  %s %s'\n"
                            % (
                                wsThisCommandResource.switch,
                                wsThisCommandResource.commandRefname,
                            )
                        )
        else:
            # Create testimport
            for wsThisModuleKeyName in parmConfiguration.pythonModuleList:
                wsModuleResourceObject = parmConfiguration.pythonModules[
                    wsThisModuleKeyName
                ]
                try:
                    wsSelfTest = getattr(
                ...
                wsF.write(
                    ...
                    % (
                        ...
                        wsModuleResourceObject.pythonModuleName,
                        wsModuleResourceObject.pythonModuleName,
                    )
                )
