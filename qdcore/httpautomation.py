...
class Crawler(object):
    ...
    def FindTableContent(self, data_tables_only=True, require_headers=True, textize=False):
        content = []
        if data_tables_only:
    ...
    def BFindStr(self, parmPattern, Ascii=False):
        for wsIx, wsThis in enumerate(parmPattern):
            wsString			= None
            if isinstance(wsThis, tuple):
                wsFunction		= 'find'
                if len(wsThis) > 1:
                    wsString		= re.compile(wsThis[1])
            else:
                wsFunction		= wsThis
            if wsIx == 0:
                ...
                if wsFunction == 'sibling':
                    ...
                    wsResult		= wsResult.find_next(wsThis[0], string=wsString)
                else:
    ...
    def MakeHtmlChild(self, parmDocument, parmTagB):
        wsNewBlock				= True
        if parmTagB.name in ['div', 'span']:
            wsNewBlock			= False
        if (parmTagB.name == 'a') and (parmDocument.linkTranslator is None):
            wsNewBlock			= False
        if wsNewBlock:
            parmDocument.Push(parmTagB.name)
        for wsThis in parmTagB.children:
            if isinstance(wsThis, bs4.element.Tag):
                self.MakeHtmlChild(parmDocument, wsThis)
            else:
                # This should be a string
                parmDocument.AppendStr(wsThis.string)
        if wsNewBlock:
            parmDocument.PopPast(parmTagB.name)

    def MakeHtml(self, parmParent, StartAfter=None, Stop=None, LinkTranslator=None):
        if parmParent is None:
            return None
        wsHtml				= bzHtml.bafSimpleDocument('html', LinkTranslator=LinkTranslator)
        if Stop is not None:
            wsStopFirstTag			= Stop[0]
            wsStopSecondTag			= Stop[1]
            wsStopString			= Stop[2]
        ...
        for wsThis in parmParent.children:
            if not wsStarted:
                if wsThis is StartAfter:
                    wsStarted			= True		# This is the price, start after this
            else:
                if Stop is not None:
                    if isinstance(wsThis, bs4.element.Tag):
                        if (wsThis.name == wsStopFirstTag):
                            wsCheck2			= wsThis.contents[0]
                            if wsCheck2.name == wsStopSecondTag:
                                if wsCheck2.string.find(wsStopString) >= 0:
                                    break
            #print id(wsThis), ": ", wsThis.__class__.__name__, " >> ", wsThis.name, " >> ", id(wsThis.parent), " >> ", wsThis
            if isinstance(wsThis, bs4.element.Tag):
                self.MakeHtmlChild(wsHtml, wsThis)
            else:
                # This should be a string
                wsStr				= wsThis.string
    ...
    def GetAndSaveBinaryFile(self, url, fn=None, Scheme=None, Binary=True, Tree=False, UseCache=True):
    ...
    def GetAndSaveTextFile(self, url, fn, Scheme=None, Binary=False, Tree=False, UseCache=False):
        self.Load(url=url, Scheme=Scheme, Binary=Binary, Tree=Tree, UseCache=UseCache)
  ...
  #
    ...
    def Load(self, url=None, Scheme=None, Binary=False, Data=None, DataGet=False, StartAfter=None,
                        Tree=True,
						ScrapeUrlsUdi=None, UseCache=False):
        """
            Load a url.
