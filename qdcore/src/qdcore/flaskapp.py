...
Generates qd_create_app.py and wsgi.py files from repos.db metadata.
Each QD Flask package declares its init function, parameters, and
priority in its qd_conf.toml flask: section. The generator reads
...
class FlaskAppGenerator:
    ...
    Reads flask_init table (populated from qd_conf.toml flask: sections)
    to produce:
      1. qd_create_app.py with qd_init_app(app) and create_app()
      2. wsgi.py for Flask/mod_wsgi deployment
    """
    ...
    def __init__(self, repo_scanner, qdsite_dpath, venv_dpath=None,
        ...
        Args:
            ...
            qdsite_prefix: Site acronym/prefix
    ...
    def generate_create_app(self):
        ...
        lines.append('Regenerated on each QdStart run from qd_conf.toml'
                     ' declarations.')
        lines.append('')
    ...
    def generate_wsgi(self):
        ...
        content = '\n'.join(lines)
        output_path = os.path.join(self.qdsite_dpath, 'wsgi.py')
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(content)
    ...
    def _format_value(self, raw_value, declared_type):
        """
        Convert a raw value (from TOML/conf) to a Python literal string.
